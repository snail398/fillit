...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/05 08:22:43 by cwing             #+#    #+#             */
/*   Updated: 2020/01/08 07:22:19 by cwing            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		free_tetris(t_shape *tetri)
{
	int y;
...
t_shape		*tetris_new(char **pos, int width, int height, char value)
{
	t_shape		*tetris;

	tetris = ft_memalloc(sizeof(t_shape));
	tetris->pos = pos;
	tetris->width = width;
...
t_shape	*get_piece(char *str, char value)
{
	t_point		*mi;
	t_point		*max;
	char		**pos;
...
t_list	*read_tetri(int fd)
{
	...
	while ((count = read(fd, buf, 21)) >= 20)
	{
		...
		ft_lstadd(&list, ft_lstnew(tetris, sizeof(t_shape)));
